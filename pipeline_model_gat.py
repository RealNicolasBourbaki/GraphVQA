...
#class RNNProgramDecoder():

from sentence_transformers import SentenceTransformer, models
class BertQuestionEncoder(torch.nn.Module):
    """
    ...
    def forward(self, questions):
        ...
        all_embeds = torch.transpose(torch.as_tensor(all_embeds), 0, 1)
...
class TransformerProgramDecoder(torch.nn.Module):
    ...
    def __init__(self, text_vocab_embedding, vocab_size, text_emb_dim, ninp, nhead, nhid, nlayers, dropout=0.1):
        ...
        self.query_embed = torch.nn.Embedding(self.num_queries, ninp) # 1, 512
    ...
    def forward(self, memory, tgt):
        ...
        ##################################
        true_batch_size = memory.size(1) # real batch: num of encoded questions here
...
class TransformerQuestionEncoder(torch.nn.Module):
    """
    The stuff that has been changed
    """
    ...
    def forward(self, src):
        ...
        src = self.emb_proj(src) * math.sqrt(self.ninp)  # why
        src = self.pos_encoder(src)
        output = self.transformer_encoder(src)
        return output


class GNNQuestionEncoder(torch.nn.Module):
    """
    A new question encoder with TransformerConv
    """
    def __init__(self, text_embeddings, edge_embeddings, n_heads, edge_feat_dim, node_feat_dim,
                 model_layers, model_top_k_every_n, dropout_rate, top_k_ratio, out_dim,
                 dense_neurons):
        super(GNNQuestionEncoder, self).__init__()
        self.text_embeddings = text_embeddings
        self.edge_embeddings = edge_embeddings
        self.edge_embedding_dim = edge_feat_dim # 300d glove
        self.node_embedding_dim = node_feat_dim
        ##################################
        ...
        self.conv1 = TransformerConv(in_channels = 300,
        ...
        self.transf1 = Lin(out_dim * n_heads, out_dim)
        self.bn1 = BatchNorm1d(out_dim)

        # Other layers
        for i in range(self.n_layers):
            self.conv_layers.append(TransformerConv(in_channels = out_dim,
                                                    out_channels = out_dim,
                                                    heads=n_heads,
                                                    dropout=dropout_rate,
                                                    edge_dim=self.edge_embedding_dim,
            ...
            self.transf_layers.append(Lin(out_dim * n_heads, out_dim))
            self.bn_layers.append(BatchNorm1d(out_dim))
            if i % self.top_k_every_n == 0:
                self.pooling_layers.append(TopKPooling(out_dim, ratio=top_k_ratio))

        # Linear layers, adjust the number of layers here. Does not make much difference though
            self.linear1 = Lin(out_dim * 2, dense_neurons)
            self.linear2 = Lin(dense_neurons, int(dense_neurons / 2))
            self.linear3 = Lin(int(dense_neurons / 2), dense_neurons)

    def forward(self,question_graphs):
        ## with positional encoding
        # x = self.text_embeddings(question_graphs.x[;,0])[:, -1, :]
        # pos = F.one_hot(question_graphs.x[;,1]), num_classes=300)
        # x = torch.add(x, pos)

        ## without positional encoding
        x = self.text_embeddings(question_graphs.x)[:, -1, :]
        edge_attr = self.edge_embeddings(question_graphs.edge_attr)[:, -1, :]
        edge_index = question_graphs.edge_index
        batch_index = question_graphs.batch
...
class PipelineModel(torch.nn.Module):
    def __init__(self):
        ...
        ##################################
        TEXT = GQATorchDataset.TEXT
        text_vocab = GQATorchDataset.TEXT.vocab  ## the glove embeddings
        ...
        ##################################
        edge_vocab = GQATorchDataset.QUESTION_EDGES_TEXT.vocab
        edge_emb_dim = 128
        self.edge_vocab_embedding = torch.nn.Embedding(text_vocab_size, edge_emb_dim)
        # self.edge_vocab_embedding.weight.data.copy_(edge_vocab.vectors)
        del edge_vocab
        ...
        self.question_encoder = GNNQuestionEncoder(text_embeddings=self.text_vocab_embedding,
                                                   edge_embeddings=self.edge_vocab_embedding,
                                                   n_heads=5,
                                                   edge_feat_dim=edge_emb_dim,
                                                   node_feat_dim=text_emb_dim,
                                                   model_layers=4,
                                                   model_top_k_every_n=1,
                                                   dropout_rate=0.2,
                                                   top_k_ratio=0.5,
                                                   out_dim=self.question_hidden_dim,
                                                   dense_neurons=self.question_hidden_dim,
                                                   )
        ##################################
    ...
    def forward(self,
        ...
        questions_encoded = self.question_encoder(question_graphs)  # [12(question length), 1(question number), 512(feature size)]
        questions_encoded = questions_encoded[None, :, :]
        ##################################
        # Decode programs
        ##################################
        # [ Len, Batch ] -> [ Len, Batch, self.question_hidden_dim ]
        ...
        # no need, these are for short answers
        ...
        x_executed = self.gat_seq(x=x_encoded,
                                  edge_index=gt_scene_graphs.edge_index,
...
if __name__ == "__main__":
    ...
    for data_batch in data_loader:
        questionID, questions, gt_scene_graphs, programs, full_answers, short_answer_label, types, question_graph = data_batch
