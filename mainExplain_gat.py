...
def main(args):
    ...
    if not args.evaluate:
        ...
        # different samplers according to different env settings.
    ...
    val_dataset = torch.utils.data.ConcatDataset(val_dataset_list) # in case more eval sets
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (data_batch) in enumerate(train_loader):
        ...
        questionID, questions, gt_scene_graphs, programs, full_answers, short_answer_label, types, question_graphs = data_batch
        del questionID
        questions, gt_scene_graphs, programs, full_answers, short_answer_label, question_graph = [
            datum.to(device=cuda, non_blocking=True) for datum in [
                questions, gt_scene_graphs, programs, full_answers, short_answer_label, question_graphs
            ]
        ] # load data to cuda
        ...
        output = model(
            ...
            programs_input,
            full_answers_input
        ) # here is where you feed the input to model
...
def validate(val_loader, model, criterion, args, FAST_VALIDATE_FLAG=False, DUMP_RESULT=False):
    ...
    # Nope, not enough space on the disk
    if DUMP_RESULT:
