...
SPLIT_TO_PROGRAMMED_QUESTION_PATH_TABLE = {
     ...
     #'val_unbiased': str(ROOT_DIR / 'GraphVQA/questions/debug_balanced_programs.json'),
...
class GQA_gt_sg_feature_lookup:
    ...
    def convert_one_gqa_scene_graph(self, sg_this):
        ...
        datum = torch_geometric.data.Data(x=x,
...
class GQATorchDataset(torch.utils.data.Dataset):
    ...
    QUESTION_EDGES_TEXT = torchtext.data.Field(sequential=True,
                                               tokenize="spacy",
                                               init_token="<start>",
                                               eos_token="<end>",
                                               include_lengths=False,
    ...
    def __init__(
        ...
        # load scene graphs and their features

        ## Reverse edges
        # with open("/mount/arbeitsdaten61/studenten3/advanced-ml/2022/gogirlspower/nicole/GraphVQA/reverse_edge.json")as obj:
        #    self.reverse_edges = json.load(obj)
        ...
        with open(programmed_question_path) as infile:
            ...
            print(programmed_question_path)
        # read in questions (programmed)
        ...
        if split in ['debug', 'train_unbiased']:
            ...
            if build_vocab_flag:
                ...
                # self.build_qa_vocab()
            self.load_qa_vocab()
            self.build_question_graph_edges_vocab()
        else:
            assert split in ['val_unbiased', 'testdev', 'val_all']
            ##################################
            # load qa vocab
    ...
    def __getitem__(self, index):
        ...
        try:
            tree = datum[11]
            # Get node features
            # node_feats, edge_feats, edge_index = self._get_features(tree)
        except IndexError:
            print(datum)
            print(self.data[0])
            exit()
        ...
        # Get graph
        ##################################
        nodes, edges, links = self._get_features(tree)  # unencoded
        ...
        return (
            ...
            full_answer_text_tokenized, short_answer_label, types, nodes, edges, links
        )

    def __len__(self):
        return len(self.data)

    def build_question_graph_edges_vocab(self):
        """
        Use torchtext vocab and glove embeddings to encode symbolic tokens in scene graph
        """
        ##################################
        # Build the vocab with tokenized dataset
        # text = SG_ENCODING_TEXT.preprocess(text)
        # # must do preprocess (tokenization) before doing the vocab building
        ##################################
        def load_str_list(fname):
            with open(fname) as f:
                lines = f.read().splitlines()
        ...
        tmp_text_list = []
        tmp_text_list += load_str_list(ROOT_DIR / 'GraphVQA/meta_info/edge_tags.txt')

        tmp_text_list.append("<self>") # add special token for self-connection
        tmp_text_list = [ tmp_text_list ]

        GQATorchDataset.QUESTION_EDGES_TEXT.build_vocab(tmp_text_list)  # no vector, just vocab

    def _get_features(self, tree):
        """
        build nodes, edges, and links for question graphs
        """
        nodes =[] # word or (word, id)
        edges = []
        links = []
        for tuple in tree:
            head = tuple[0].strip()
            ...
            if sub not in nodes:
                nodes.append(sub)
            edges.append(edge)
            links.append([head, sub])
            ###################
            ## when bidir edges are needed
            ###################
            # reverse_edge = self.reverse_edges[edge]
            # edges.append(reverse_edge)
            # links.append([sub, head])
        # for node in nodes:
            # edges.append(self.reverse_edges["self_edge"])
            # links.append([sub, sub])
            # edges.append(self.reverse_edges["self_edge"])
            # links.append([head, head])
    ...
    def build_qa_vocab(self):
        ...
        for datum in self.data:
            question_text = datum[1]
            program_text_tokenized = datum[6]  # [select(sky), verify([0]), dark] something like that
            full_answer_text = datum[5]
            # Question
            # must do preprocess (tokenization) before doing the buld vocab
            question_text_tokenized = GQATorchDataset.TEXT.preprocess(question_text)
        ...
        # vocab for questions, programs, full_answers
        del tmp_text_list
...
def GQATorchDataset_collate_fn(data):
    ...
    """
    questionID, question_text_tokenized, sg_datum, program_text_tokenized,\
    full_answer_text_tokenized, short_answer_label, types, nodes_batch, edges_batch, links_batch \
        = zip(*data)

    question_text_processed = GQATorchDataset.TEXT.process(question_text_tokenized)

    ######################################
    ## only in  BERT settings, replace tokens with BERT compatible ones
    ######################################
    # question_text_tokenized_padded = GQATorchDataset.TEXT.pad(question_text_tokenized)
    # for i, q in enumerate(question_text_tokenized_padded):
        # for j, t in enumerate(q):
            # if t == "<start>":
                # question_text_tokenized_padded[i][j] = "[CLS]"
            # elif t == "<end>":
                # question_text_tokenized_padded[i][j] = "[SEP]"
            # elif t == "<pad>":
                # question_text_tokenized_padded[i][j] = "[PAD]"
    ...
    ####################################
    # process all nodes, edges, and links
    ####################################
    questions_graphs_batch = list()
    for i in range(0, len(questionID)):
        new_nodes = []
        node_dict = {}
        for nodes in nodes_batch:
            for node in nodes:
                node = GQATorchDataset.TEXT.vocab[node]
                #############
                ## when positional info is needed
                #############
                # node = GQATorchDataset.TEXT.vocab[node[0]]
                node_dict[node] = len(new_nodes)
                new_nodes.append([node])
                #############
                ## when positional info is needed
                #############
                # new_nodes.append([node, node[1]]) # positional info
        nodes = torch.tensor(new_nodes)

        new_edges = []
        for edges in edges_batch:
            for edge in edges:
                edge = GQATorchDataset.QUESTION_EDGES_TEXT.vocab[edge]
                new_edges.append([edge])
        edges = torch.tensor(new_edges)

        new_links = []
        for links in links_batch:
            for link in links:
                from_i = node_dict[GQATorchDataset.TEXT.vocab[link[0]]]
                to_j = node_dict[GQATorchDataset.TEXT.vocab[link[1]]]
                new_links.append([from_i, to_j])
        links = torch.tensor(new_links, dtype=torch.long)
        question_graph = torch_geometric.data.Data(x=nodes,
                                                   edge_index=links.t().contiguous(),
                                                   edge_attr=edges)
        questions_graphs_batch.append(question_graph)

    ###########################################
    # make nodes edges links into PyG dataset
    ###########################################

    question_graph_processed = torch_geometric.data.Batch.from_data_list(questions_graphs_batch)

    return (
        questionID, question_text_processed, sg_datum_processed, program_text_tokenized_processed,
        full_answer_text_tokenized_processed, short_answer_label, types, question_graph_processed #, question_text_tokenized_padded # for BERT
    )
